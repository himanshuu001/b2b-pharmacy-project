...
function filterRetailers() {
  ...
  const state = document.getElementById('ret-state-filter')?.value || '';
  ...
  renderRetailers(allRetailers.filter(d =>
    ...
    (!state || d.city.toLowerCase().includes(state.toLowerCase())) &&
    (!type || d.type === type)
  ));
}
...
function renderProducts(data) {
  ...
  tbody.innerHTML = data.map(d => {
    const thumb = d.image_url
      ? `<img src="${d.image_url}" alt="" style="width:36px;height:36px;object-fit:cover;border-radius:6px;border:1px solid var(--border);display:inline-block;vertical-align:middle;margin-right:8px">`
      : `<span style="display:inline-block;width:36px;height:36px;background:var(--pri-light);border-radius:6px;vertical-align:middle;margin-right:8px;font-size:18px;line-height:36px;text-align:center">💊</span>`;
    return `<tr>
      <td><div style="display:flex;align-items:center">${thumb}<strong>${d.name}</strong></div></td>
      <td style="font-family:'DM Mono',monospace;font-size:12px">${d.sku}</td>
      <td><span class="badge badge-blue">${d.category}</span></td>
      <td>
        <div style="font-weight:500">₹${d.mrp}</div>
        ${d.price_to_distributor ? `<div style="font-size:11px;color:var(--text-3)">D: ₹${d.price_to_distributor}</div>` : ''}
        ${d.price_to_retailer ? `<div style="font-size:11px;color:var(--text-3)">R: ₹${d.price_to_retailer}</div>` : ''}
      </td>
...
async function editProduct(id) {
  try {
    ...
    form.querySelector('[name=product-dist-price]').value = product.price_to_distributor || '';
    form.querySelector('[name=product-ret-price]').value = product.price_to_retailer || '';
    ...
    document.getElementById('productExistingImage').value = product.image_url || '';
...
async function saveProduct() {
  ...
  fd.append('name', form.querySelector('[name=product-name]').value);
  fd.append('sku', form.querySelector('[name=product-sku]').value);
  fd.append('category', form.querySelector('[name=product-cat]').value);
  fd.append('mrp', form.querySelector('[name=product-mrp]').value);
  fd.append('price_to_distributor', form.querySelector('[name=product-dist-price]').value || '');
  fd.append('price_to_retailer', form.querySelector('[name=product-ret-price]').value || '');
  fd.append('manufacturer', form.querySelector('[name=product-mfr]').value || 'Fair Ford Pharma');
  fd.append('stock', form.querySelector('[name=product-stock]').value || 0);
  fd.append('description', form.querySelector('[name=product-desc]').value);
  fd.append('status', form.querySelector('[name=product-status]').value);
  if (imageFile) fd.append('image', imageFile);

  try {
    const url = productId ? `${API_BASE}/products/${productId}` : `${API_BASE}/products`;
    const res = await fetch(url, { method: productId ? 'PUT' : 'POST', body: fd });
    if (!res.ok) {
      const body = await res.json().catch(() => ({ error: res.statusText }));
...
async function loadSchemes() {
  ...
  try {
    ...
    list.innerHTML = '';
    if (!data.length) {
      list.innerHTML = '<div style="grid-column:1/-1;text-align:center;padding:32px;color:var(--text-3)">No active schemes found</div>';
      return;
    }
    const borderColors = { active: 'var(--green)', upcoming: 'var(--amber)' };
    const progClass = { active: 'prog-green', upcoming: '' };
    list.innerHTML = data.map(s => {
      ...
      const badgeClass = s.status === 'active' ? 'badge-green' : 'badge-amber';
      const statusLabel = s.status === 'active' ? 'Live' : 'Upcoming';
      ...
      const desc = (s.description || s.type + ' · ' + s.category).replace(/</g, '&lt;').replace(/>/g, '&gt;');
      return `<div class="card" style="border-top:3px solid ${bc}">
        <div class="card-header">
          <span class="card-title">${name}</span>
          <div style="display:flex;align-items:center;gap:6px">
            <span class="badge ${badgeClass}">${statusLabel}</span>
            <button class="btn btn-ghost" style="padding:3px 10px;font-size:12px;height:auto;line-height:1.4" onclick="openEditScheme(${s.id})">Edit</button>
            <button style="padding:3px 10px;font-size:12px;background:var(--red-bg);color:var(--red);border:none;border-radius:6px;cursor:pointer;font-weight:500;line-height:1.4" onclick="deleteScheme(${s.id})">Remove</button>
          </div>
        </div>
        <div class="card-body">
          <div style="font-size:12px;color:var(--text-2);margin-bottom:10px">${desc}</div>
...
function openEditScheme(id) {
  ...
  document.getElementById('editSchemeName').value = s.name;
  document.getElementById('editSchemeType').value = s.type;
  document.getElementById('editSchemeCat').value = s.category;
  document.getElementById('editSchemeChannel').value = s.channel;
  document.getElementById('editSchemeStart').value = s.start_date;
  document.getElementById('editSchemeEnd').value = s.end_date;
  document.getElementById('editSchemeDesc').value = s.description || '';
  document.getElementById('schemeEditModal').classList.add('active');
}

function closeEditSchemeModal() {
  document.getElementById('schemeEditModal').classList.remove('active');
}

async function saveEditScheme(e) {
  e.preventDefault();
  const id = document.getElementById('editSchemeId').value;
  const payload = {
    name: document.getElementById('editSchemeName').value.trim(),
    type: document.getElementById('editSchemeType').value,
    category: document.getElementById('editSchemeCat').value,
    channel: document.getElementById('editSchemeChannel').value,
    start_date: document.getElementById('editSchemeStart').value,
    end_date: document.getElementById('editSchemeEnd').value,
    description: document.getElementById('editSchemeDesc').value.trim(),
  };
  try {
    await apiFetch(`/schemes/${id}`, { method: 'PUT', body: JSON.stringify(payload) });
...
async function deleteScheme(id) {
  if (!confirm('Remove this scheme? This cannot be undone.')) return;
  try {
    await apiFetch(`/schemes/${id}`, { method: 'DELETE' });
    await loadSchemes();
    toast('Scheme removed.');
  } catch (e) { toast(e.message, 'error'); }
}
